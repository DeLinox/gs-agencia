...
/*
    ...
    buscarCliente = function (dlg) {
        $(dlg).find('form').submit(function () {
            ...
            $("input#cliente").val( $(this).find('#sclie_id').val() )
    ...
    $('.searchclient').click(function () {
        var tipo = $('#movi_clase').val() == '1' ? 'Proveedor' : 'Cliente';
        $(this).load_dialog({
            title: $(this).attr('title'),
            script: baseurl + 'assets/js/'+ tipo +'/search.js?v=3',
    ...
    $('#completar').click(function(){
        var tipo = $('#movi_clase').val() == '1' ? 'Proveedores' : 'Clientes';
        doc_num = $('#docnum').val();
        $.ajax({
          dataType: "json",
          url: baseurl + tipo + "/buscar",
          data: { 
                q :'num',
                num: doc_num 
            },
          success: function(e){
                if (e.total_count == 1) {
                    ...
                    $("input#cliente").val(e.items[0].id);
    ...
    $.each($('.sortable .fact-item [dval]'),function(index,item){
     ...
     $(form).formPost(true,{},function(data){
         if(data.exito==true){
            ...
            else window.location.href = $('#baseurl').val()+'Almacen/listado/'+data.tipo+"#2";
         }else{
            $.each($('.sortable .fact-item [dval]'),function(index,item){
                temp = $(item).val();
