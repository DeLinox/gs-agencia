...
$(document).on('ready', function() {
    ...
    function botones(id, permisos, $ar) {
        html = `
        <span class='show_correo oculto'><a href='{baseurl}Contacto/eliminar_hote/{id}' class='btn btn-danger btn-sm eliminar'><span class='glyphicon glyphicon-trash'></span></a></span>
        <span class="show_editar oculto"><a href='{baseurl}Contacto/crear_hote/{id}' class='btn btn-success btn-sm editar'><span class='glyphicon glyphicon-pencil'></span></a></span>`;
        ...
        $ar.find('.eliminar').click(function() {
            if (confirm("è¢ƒDesea eliminar el hotel?")) {
                $.gs_loader.show();
                $.ajax({
                    type: "POST",
                    dataType: "json",
