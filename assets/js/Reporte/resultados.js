...
$(document).on('ready', function() {
    ...
    */
    ...
    $('.actualiza').click(function() {
        $(this).load_dialog({
            title: 'Registrar Periodo',
            custom_url: $(this).attr('href')+"&anio="+$('select[name="anio"]').val()+"&mes="+$('select[name="mes"]').val()+"&tipo="+$('select[name="tipo"]').val(),
            loaded: function($dlg) {
                $dlg.find('form').submit(function() {
                    $(this).formPost(true, {}, function(data) {
                        alert(data.mensaje);
...
function actualizar($this){
    var url = baseurl+'Reporte/actualiza_resultados';
...
function llenarTabla($this) {
    ...
    $.getJSON(url, $this.serialize(),function(data) {
        $.gs_loader.hide();
        $('#tbl-resultados').html(data.tabla);
        if(data.existe == "SI"){
            $('.actualiza').show();
        }else{
            $('.actualiza').hide();
