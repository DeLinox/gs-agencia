...
class Liquidacion extends CI_Controller {
	...
	function guardar($id = ''){
		...
		if(empty($id)){
            ...
            $this->Model_general->add_log("CREAR",6,"Creacion de Hoja de liquidación LIQ-".str_pad($numero, 8, "0", STR_PAD_LEFT));
            ...
            $this->Model_general->add_log("EDITAR",6,"Edición de Hoja de liquidación LIQ-".str_pad($numero, 8, "0", STR_PAD_LEFT));
            $this->db->trans_commit();
        }
    ...
    */
	...
	public function eliminar($liqu_id = ''){
        ...
        if($liquidacion->liqu_estado == 'PAGADO' || $liquidacion->liqu_cobrado > 0){
            ...
            $this->Model_general->add_log("ELIMINAR",6,"Eliminación de Hoja de liquidación LIQ-".$liquidacion->liqu_numero);
    ...
    public function guardar_cobro($id = ''){
        ...
        if($liq->liqu_estado == 'PAGADO'){
            $json['exito'] = false;  
            $json['mensaje'] = "La liquidación ya esta cobrada";
            echo json_encode($json);
            exit(0);   
        }
        $this->db->trans_start();
        ...
        $this->Model_general->add_log("COBRO",6,"Cobro de Hoja de liquidación ".$liq->numero." ".$pagado." / ".$total." ".$moneda.", Código de caja: ".$codigo_cuen);
	...
	public function cancelar_cobroLiquidacion($id = ''){
		...
		if($mov->num_rows() > 0){
			...
			$this->Model_general->add_log("COBRO",6,"Anulacion de cobro de Hoja de liquidación ".$liq->numero." ".$liq->liqu_total." ".$liq->liqu_moneda.", Código de caja: ANULADO");
    ...
    public function liqu_genReceptivo(){
        ...
        $datos["titulo"] = "Generar Hoja de liquidación (receptivo)";
        $this->load->helper('Funciones');
        $this->load->model("Model_general");
        $this->load->library('Ssp');
        $this->load->library('Cssjs');
    ...
    public function ldet_guardar($deta_id = ''){
        ...
        if($this->Model_general->guardar_edit_registro("liquidacion_detalle", $datas, $where)){
            ...
            if(!$this->actualiza_precios($deta_id)){
                $this->db->trans_rollback();
                $resp["exito"] = false;
                $resp["mensaje"] = "Ocurrio un error, intentelo más tarde(act precios).";
            ...
            if(isset($adic_precio)){
                if(!$this->addAdiciones($adic_precio, $adic_name, "ADICION", $deta_id, $adic_id)){
                    $this->db->trans_rollback();
                    $resp["exito"] = false;
                    $resp["mensaje"] = "Ocurrio un error, intentelo más tarde(adicioes).";
                    $this->Model_general->dieMsg($resp);
                }
            }
            if(isset($desc_precio)){
                if(!$this->addAdiciones($desc_precio, $desc_name, "DESCUENTO", $deta_id, $desc_id)){
                    $this->db->trans_rollback();
                    $resp["exito"] = false;
                    $resp["mensaje"] = "Ocurrio un error, intentelo más tarde(descuento).";
                    $this->Model_general->dieMsg($resp);
                }
            }
        }else{
            $this->db->trans_rollback();
            $resp["exito"] = false;
            $resp["mensaje"] = "Ocurrio un error, intentelo más tarde(general).";
            $this->Model_general->dieMsg($resp);
        }
        $this->db->trans_commit();
        $resp["exito"] = true;
    ...
    public function guardar_orden_pago($liqu_id = ''){
        ...
        if($liqu_id != ''){
                ...
                foreach ($paquetes as $paq) {
                    ...
                    if(($lpaq = $this->Model_general->guardar_registro("liquidacion_paqu", $liq_paq))){
                        if( !$this->addDetalles_liquidacion(explode(',', $paq->detas), $lpaq["id"],$liq["id"],$paq->paqu_nombre) ){
                            $this->db->trans_rollback();
                            $resp["exito"] = false;
                            $resp["mensaje"] = "Ocurrio un error, intenlo más tarde";
                            $this->Model_general->dieMsg($resp);
                        }
                    }else{
                        $this->db->trans_rollback();
            ...
            $this->Model_general->add_log("CREAR",6,"Creación de Hoja de liquidación LIQ-".str_pad($numero, 8, "0", STR_PAD_LEFT));
        }
        $resp["exito"] = true;
